...
Summary:
アプリケーション全体で使用する定数を一元管理するファイル。
主に、データベースのカラム名（キー）と、ユーザー画面やLLMへのプロンプト入力時に使用する「日本語の表示ラベル」を紐づけるマッピング辞書（`PATIENT_FIELD_LABELS`）を定義しています。また、チェックボックスとそれに付随する詳細テキスト入力欄の関連付け（`CHECKBOX_TEXT_PAIRS`）も管理しており、計画書の膨大なデータ項目をシステムが正しく解釈・表示するための辞書としての役割を担っています。

Tags: Constants, Mapping, Labels, UI, LLM Context
"""

# DBカラム名と日本語ラベルのマッピング
# LLMへの入力コンテキスト構築時などに使用
PATIENT_FIELD_LABELS = {
    ...
    # ADL・基本動作 (旧リポジトリにはなかったが、新スキーマで必要な追加分)
    "adl_equipment_and_assistance_details_txt": "ADL補装具・介助詳細",
}
