...
従来の感情分析（Sentiment Analysis）ではなく、**含意関係認識（NLI）**モデルを採用し、**ONNX Runtime**による量子化・高速化を行うことで、「精度」と「速度」を両立する。
...
検証の結果、ベクトル解析や量子化AIは「文法構造の無視」や「意味の混同」により不適格となり、最終的に**「Aspect Rule V2.2」** が採用された。
...
| 量子化AI | 8. Quantized BERT | BERT (int8 Quantization)<br>日本語感情分析モデルを動的量子化し、CPU推論を高速化。 | × 意味混同<br>処理速度はルールベースに迫るが、「足の痛み」をPositive(0.95)と誤判定**する致命的な欠陥を確認。身体/精神の分離不可。 | 不採用 |
